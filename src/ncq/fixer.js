...
class Fixer{
    ...
    attemptFix(snippet, parse){
        ...
        if(parse){
            ...
            var errors = LinterHandler.errors(snippet.errors)
            if(!errors || errors.length < 1) return {snippet, fixed};
        }
        ...
        for(var e of LinterHandler.errors(snippet.errors)){
    ...
    fix(snippet){
        ...
        if(!this.deleteMode) return snippet;
